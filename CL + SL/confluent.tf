data "confluent_environment" "primary" {
  id = var.cc_env_primary
}

data "confluent_environment" "secondary" {
  id = var.cc_env_secondary
}
...
data "confluent_schema_registry_cluster" "secondary" {
  environment {
    id = var.cc_env_secondary
  }
}
...
resource "confluent_kafka_cluster" "secondary" {
  ...
  environment {
    id = data.confluent_environment.secondary.id
  }
...
resource "confluent_api_key" "sr-api-key-primary" {
  display_name = "primary-active-passive-sr--api-key"
  description  = "SR API Key to manage the primary cluster"
  owner {
    id          = data.confluent_service_account.default.id
    api_version = data.confluent_service_account.default.api_version
    kind        = data.confluent_service_account.default.kind
...
resource "confluent_api_key" "sr-api-key-secondary" {
  display_name = "secondary-active-passive-kafka--api-key"
  description  = "SR API Key to manage the secondary cluster"
  owner {
    id          = data.confluent_service_account.default.id
    api_version = data.confluent_service_account.default.api_version
    kind        = data.confluent_service_account.default.kind
  ...
  managed_resource {
    id          = data.confluent_schema_registry_cluster.secondary.id
    api_version = data.confluent_schema_registry_cluster.secondary.api_version
    kind        = data.confluent_schema_registry_cluster.secondary.kind
...
resource "confluent_role_binding" "all-subjects-primary-rb" {
  principal   = "User:${data.confluent_service_account.default.id}"
  role_name   = "ResourceOwner"
  crn_pattern = "${data.confluent_schema_registry_cluster.primary.resource_name}/subject=*"
}

resource "confluent_role_binding" "all-subjects-secondary-rb" {
  principal   = "User:${data.confluent_service_account.default.id}"
  role_name   = "ResourceOwner"
  crn_pattern = "${data.confluent_schema_registry_cluster.secondary.resource_name}/subject=*"
}

resource "confluent_schema_exporter" "primary" {
  ...
  credentials {
    key    = confluent_api_key.sr-api-key-primary.id
    secret = confluent_api_key.sr-api-key-primary.secret
  }
  ...
  destination_schema_registry_cluster {
    rest_endpoint = data.confluent_schema_registry_cluster.secondary.rest_endpoint
    credentials {
      key    = confluent_api_key.sr-api-key-secondary.id
      secret = confluent_api_key.sr-api-key-secondary.secret
    }
  }
  depends_on = [ confluent_role_binding.all-subjects-primary-rb, confluent_role_binding.all-subjects-secondary-rb ]
  reset_on_update = true
}



resource "confluent_schema_exporter" "secondary" {
